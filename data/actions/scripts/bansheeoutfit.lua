function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local demonKnightLookType = 652
	local ownerStorage = 1996799 -- usado no player pra salvar que ele é o dono
	...
	if not ownerGuid or ownerGuid == "" then
		...
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You are now the exclusive owner of this Banshee artifact.")
	end

	-- Verifica se o player é o dono do item
	if tostring(player:getGuid()) ~= ownerGuid then
		player:sendCancelMessage("Only " .. (ownerName or "the original owner") .. " can use this Banshee artifact.")
		return true
	end

	-- Verifica se já está transformado
	if player:getOutfit().lookType == demonKnightLookType then
		player:sendCancelMessage("You are already transformed into a Banshee.")
		return true
	end
	...
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You transformed into a Banshee!")
