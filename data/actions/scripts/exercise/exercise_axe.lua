...
local skills = {
    [8247] = {id = SKILL_AXE, voc = 4, range = 26},
}
...
-- Função para iniciar o treinamento de axe
local function startTrainAxe(playerId, itemid, fpos, start_pos, rateMagic, rateSkill)
    ...
    if exercise and exercise:hasAttribute(ITEM_ATTRIBUTE_CHARGES) then
        ...
        if charges_n >= 1 then
            ...
            if charges_n == 1 then
                exercise:remove(1)
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Axe Training completed.")
                -- Remove proteção contra push
                if removePushProtection then
                    removePushProtection(player)
                end
                ...
                addEvent(startTrainAxe, player:getVocation():getAttackSpeed(), playerId, itemid, fpos, start_pos, rateMagic, rateSkill)
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Axe Training. Weapon charges: " .. (charges_n - 1))
            end
        else
            exercise:remove(1)
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Axe Training Finished.")
            -- Remove proteção contra push
            if removePushProtection then
                removePushProtection(player)
            end
    ...
    if target:isItem() and isInArray(dummies, target:getId()) then
        ...
        if vocId == 0 or vocId == skills[item.itemid].voc or vocId == skills[item.itemid].voc + 4 then
            ...
            if GlobalTrainingControl then
                local success, currentTraining = GlobalTrainingControl.startTraining(player:getId(), GlobalTrainingControl.TRAINING_TYPES.SKILL_AXE)
                if not success then
                ...
                if playersInTraining[player:getId()] then
                    player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "You are already in Axe training.")
                    return false
                end
            end
            ...
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "You started Axe training.")
            addEvent(startTrainAxe, 0, player:getId(), item.itemid, target:getPosition(), start_pos, rateMagic, rateSkill)
        else
