...
local skills = {
    [8246] = {id = SKILL_CLUB, voc = 4, range = 27},
}
...
-- Função para iniciar o treinamento de club
local function startTrainClub(playerId, itemid, fpos, start_pos, rateMagic, rateSkill)
    ...
    if pos_n:getDistance(start_pos) > 0 then
        player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Club Training interrupted.")
    ...
    if exercise and exercise:hasAttribute(ITEM_ATTRIBUTE_CHARGES) then
        ...
        if charges_n >= 1 then
            ...
            if charges_n == 1 then
                exercise:remove(1)
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Club Training completed.")
                -- Remove proteção contra push
                if removePushProtection then
                    removePushProtection(player)
                end
                ...
                addEvent(startTrainClub, player:getVocation():getAttackSpeed(), playerId, itemid, fpos, start_pos, rateMagic, rateSkill)
                player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Club Training. Weapon charges: " .. (charges_n - 1))
            end
        else
            exercise:remove(1)
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "Club Training Finished.")
            -- Remove proteção contra push
            if removePushProtection then
                removePushProtection(player)
            end
    ...
    if target:isItem() and isInArray(dummies, target:getId()) then
        ...
        if vocId == 0 or vocId == skills[item.itemid].voc or vocId == skills[item.itemid].voc + 4 then
            ...
            if GlobalTrainingControl then
                local success, currentTraining = GlobalTrainingControl.startTraining(player:getId(), GlobalTrainingControl.TRAINING_TYPES.SKILL_CLUB)
                if not success then
                ...
                if playersInTraining[player:getId()] then
                    player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "You are already in Club training.")
                    return false
                end
            end
            ...
            player:sendTextMessage(MESSAGE_STATUS_CONSOLE_BLUE, "You started Club training.")
            addEvent(startTrainClub, 0, player:getId(), item.itemid, target:getPosition(), start_pos, rateMagic, rateSkill)
        else
