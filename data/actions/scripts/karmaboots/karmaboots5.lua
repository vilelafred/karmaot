function onUse(cid, item, fromPosition, itemEx, toPosition)
    local rewardStorage = 5699
    local requiredItem = 8321     -- Karma Boots original
    local rewardItem = 6488     -- Karma Boots +4
    ...
    if getPlayerItemCount(cid, requiredItem) < 1 then
        doPlayerSendCancel(cid, "You need the Karma Boots +4 to receive this upgrade.")
    ...
    if doPlayerRemoveItem(cid, requiredItem, 1) then
        if doPlayerAddItem(cid, rewardItem, 1) then
            ...
            doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Your Karma Boots have been successfully upgraded to +5!")
