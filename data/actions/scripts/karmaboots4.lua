function onUse(cid, item, fromPosition, itemEx, toPosition)
    local rewardStorage = 5099
    local requiredItem = 8240     -- Karma Boots original
    local rewardItem = 8321      -- Karma Boots +4
    ...
    if getPlayerItemCount(cid, requiredItem) < 1 then
        doPlayerSendCancel(cid, "You need the Karma Boots +3 to receive this upgrade.")
    ...
    if doPlayerRemoveItem(cid, requiredItem, 1) then
        if doPlayerAddItem(cid, rewardItem, 1) then
            ...
            doPlayerSendTextMessage(cid, MESSAGE_INFO_DESCR, "Your Karma Boots have been successfully upgraded to +4!")
