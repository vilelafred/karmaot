function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	local demonKnightLookType = 773
	local ownerStorage = 1004499 -- usado no player pra salvar que ele é o dono
	...
	if not ownerGuid or ownerGuid == "" then
		...
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You are now the exclusive owner of this Lory Outfit artifact.")
	end

	-- Verifica se o player é o dono do item
	if tostring(player:getGuid()) ~= ownerGuid then
		player:sendCancelMessage("Only " .. (ownerName or "the original owner") .. " can use this Lory artifact.")
		return true
	end

	-- Verifica se já está transformado
	if player:getOutfit().lookType == demonKnightLookType then
		player:sendCancelMessage("You are already transformed into a Lory.")
		return true
	end
	...
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You transformed into a Lory!")
