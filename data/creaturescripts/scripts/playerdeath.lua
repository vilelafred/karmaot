    ...
    if byPlayer == 1 then
        ...
        if skullTime > 0 then
            ...
            if kills >= killsToBan then
                ...
                -- Verifica se o jogador está na tabela players antes de baní-lo
                local playerName = killer:getName()
                local queryResult = db.storeQuery("SELECT `id` FROM `players` WHERE `name` = " .. db.escapeString(playerName))

                if queryResult ~= false then
                    local playerId = result.getDataInt(queryResult, "id")
                    result.free(queryResult)

                    if playerId ~= false then
                        local timeNow = os.time()
                        db.query("INSERT INTO `account_bans` (`account_id`, `reason`, `banned_at`, `expires_at`, `banned_by`) VALUES (" .. killer:getAccountId() .. ", " .. db.escapeString("Excessive Unjustified Player Killing.") .. ", " .. timeNow .. ", " .. timeNow + (killsBanLenght * 86400) .. ", " .. playerId .. ")")
