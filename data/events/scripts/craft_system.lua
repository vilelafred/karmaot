...
function Player:sendVocCraftWindow(config, lastChoice)
    local function buttonCallback(button, choice)   
      ...
      local function sendCraftResult(self, config, lastChoice, choice)
        local item = config.system[lastChoice].items[choice.id].item
        local count = config.system[lastChoice].items[choice.id].count or 1

        if count then
          self:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have crafted x"..count.." ["..item.."].")
        else
          self:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have crafted ["..item.."].")
        end
      end

      -- ... resto da função a partir daqui ...

      self:addItem(config.system[lastChoice].items[choice.id].itemID, config.system[lastChoice].items[choice.id].count)
      sendCraftResult(self, config, lastChoice, choice) -- Chamada da nova função sendCraftResult
      self:getPosition():sendMagicEffect(6)
    end
  end
