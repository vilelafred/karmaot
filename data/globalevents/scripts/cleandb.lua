...
local inactiveMonthsToDeleteCharacter = 3 -- quantos meses o player precisa ficar inativo para ser deletado.
local emptyAccountMonths = 3 -- quantos meses uma conta precisa ficar sem nenhum character criado para ser deletada.
...
local function doCheckInactivePlayer()  -- deleta automaticamente o dados das tabelas "houses, player_items, player_depotitems, player_deaths, guilds, player_storage"
    ...
    local fromClause = "`players` WHERE `group_id` = 1 AND lastlogin <= " .. timeStamp .. " AND `name` != 'Rook Sample'"
