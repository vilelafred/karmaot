...
function doPlayerGiveItem(cid, itemid, count, charges)
	local hasCharges = (ItemType(itemid):isRune() == true or isItemFluidContainer(itemid) == true)
...
function doPlayerTakeItem(cid, itemid, amount)
	return getPlayerItemCount(cid, itemid) >= amount and Player(cid):removeItem(itemid, amount, -1, true)
end
...
function getPlayerVipPoints(cid)
	local player = Player(cid)
    local Info = db.storeQuery("SELECT `premium_points` FROM `accounts` WHERE `id` = " .. player:getAccountId() .. "")
        local Points = result.getDataInt(Info, "premium_points")
		result.free(Info)
        return tonumber(Points) or Points
end
...
function doPlayerAddVipPoints(cid, points)
    ...
    if dif >= 0 then
        db.executeQuery("UPDATE `accounts` SET `premium_points` = `premium_points` + " .. points .. " WHERE `id` = " .. player:getAccountId() .. ";")
        return TRUE
    end
    return FALSE
end

function doPlayerAddHistoryNugget(cid, points)
	local player = Player(cid)
	local accountId = player:getAccountId()
	local playerName = db.escapeString(player:getName())
    db.query("INSERT INTO `transaction_history` (`account_id`, `player`, `points`) VALUES ("..accountId..", "..playerName..", "..points..");")
    return true
end

function doPlayerRemoveVipPoints(cid, points)
	local player = Player(cid)
    local dif1 = getPlayerVipPoints(cid) - points
    if dif1 >= 0 then
...
function getPpts(cid)
	local player = Player(cid)
    local check6 = db.storeQuery("SELECT `premium_points` FROM `accounts` WHERE `id` = " .. player:getAccountId() .. "")
        local Points = result.getDataInt(check6, "premium_points")
		result.free(check6)
        return tonumber(Points) or Points
end

function addPpts(cid,amount)
local player = Player(cid)
db.executeQuery("UPDATE accounts SET premium_points = "..getPpts(cid).."+"..amount.." WHERE id = ".. player:getAccountId()) end
function removePpts(cid,amount)
local player = Player(cid)
db.executeQuery("UPDATE accounts SET premium_points = "..getPpts(cid).."-"..amount.." WHERE id = ".. player:getAccountId()) end
function setPpts(cid,value)
local player = Player(cid)
db.executeQuery("UPDATE accounts SET premium_points = "..value.." WHERE id = ".. player:getAccountId()) end

function changePpts(cid)
if getPpts(cid) >= 1 then
doPlayerSendCancel(cid, "##system##PPTS,"..getPpts(cid).."")
doPlayerSendCancel(cid, "")
else
doPlayerSendCancel(cid, "##system##PPTS,0")
doPlayerSendCancel(cid, "")
end
end
...
function getPlayerInstantSpellCount(cid) local p = Player(cid) return #p:getInstantSpells() end
...
function getLastLogout(player)
	local rows = db.storeQuery("SELECT `lastlogout` FROM `players` WHERE `id` = " .. player .. ";")
	if rows ~= false then
		local on = result.getDataInt(rows, "lastlogout")
		result.free(rows)
		return on
	end
end

function listRunesStats()
	local druid = {2, 6}
	local paladin = {3, 7}
	local sorcerer = {1, 5}
	local druidAndSorcerer = {1,2,5,6}
	local druidSorcPala = {1,2,3,5,6,7}
	local runes = {
		["cure poison"] = {sto = 95000, voc = druid},
		["animate dead"] = {sto = 95001, voc = druidAndSorcerer},
		["chameleon"] = {sto = 95002, voc = druid},
		["convince creature"] = {sto = 95003, voc = druid},
		["disintegrate"] = {sto = 95004, voc = druidSorcPala},
		["destroy field"] = {sto = 95005, voc = druidSorcPala},
		["explosion"] = {sto = 95006, voc = druidAndSorcerer},
		["energy wall"] = {sto = 95007, voc = druidAndSorcerer},
		["energy bomb"] = {sto = 95008, voc = sorcerer},
		["energy field"] = {sto = 95009, voc = druidAndSorcerer},
		["fireball"] = {sto = 95010, voc = sorcerer},
		["fire wall"] = {sto = 95011, voc = druidAndSorcerer},
		["fire bomb"] = {sto = 95012, voc = druidAndSorcerer},
		["fire field"] = {sto = 95013, voc = druidAndSorcerer},
		["great fireball"] = {sto = 95014, voc = sorcerer},
		["heavy magic missile"] = {sto = 95015, voc = druidAndSorcerer},
		["intense healing rune"] = {sto = 95016, voc = druid},
		["light magic missile"] = {sto = 95017, voc = druidAndSorcerer},
		["magic wall"] = {sto = 95018, voc = sorcerer},
		["poison wall"] = {sto = 95019, voc = druidAndSorcerer},
		["poison bomb"] = {sto = 95020, voc = druid},
		["poison field"] = {sto = 95021, voc = druidAndSorcerer},
		["paralyze"] = {sto = 95022, voc = druid},
		["sudden death"] = {sto = 95023, voc = sorcerer},
		["soulfire"] = {sto = 95024, voc = druidAndSorcerer},
		["ultimate healing"] = {sto = 95025, voc = druid}
	}

	return runes
end

runes = listRunesStats()

function listMunitionsStats()
	local munitionsStats = {
		["conjure arrow"] = {qtd = 20, itemId = 2544, sto = 96000, cap = 0.7},
		["conjure poisoned arrow"] = {qtd = 10, itemId = 2545, sto = 96001, cap = 0.8},
		["conjure bolt"] = {qtd = 10, itemId = 2543, sto = 96002, cap = 0.8},
		["conjure explosive arrow"] = {qtd = 10, itemId = 2546, sto = 96003, cap = 0.9},
		["conjure power bolt"] = {qtd = 3, itemId = 2547, sto = 96004, cap = 0.9},
	}

	return munitionsStats
end

munitionsStats = listMunitionsStats()

function getMunitionStatsFromSpellName(arr, key)
...
function listMunitionsPaladin()
	local munitionsPaladin = {"Power Bolt", "Explosive Arrow", "Conjure Bolt", "Poisoned Arrow", "Conjure Arrow"}
	return munitionsPaladin
end

function verifyStringExistsOnArray(str, arr)
    for i = 1, #arr do
        if arr[i] == str then
            return true
        end
...
function findRuneName(runeName)
...
function findMunitionName(munitionName)
	for k, v in pairs(munitionsStats) do
		if k == munitionName then
			return true
		end
	end
	return false
...
function capitalizeWords(str)
    local result = str:gsub("(%a)([%w_']*)", function(firstLetter, rest)
...
function formatTime(min)
	local days = math.floor(min / (24 * 60))
	local hours = math.floor((min / 60) % 24)
	local min = min % 60
...
function doPlayerAddItem(cid, itemid, count)
	return cid:addItem(itemid, count)
end

function doAddContainerItem(container, itemid, count)
	local container = Container(container.uid)
