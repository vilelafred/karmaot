...
function Player.getLossPercent(self)
	...
	-- Check VIP blessing (6)
	local hasVipBlessing = self:hasBlessing(6)
	
	local lossPercent = {
		[0] = 100,  -- No blessings = 100% loss
		[1] = 70,   -- 1 blessing = 70% loss
		[2] = 45,   -- 2 blessings = 45% loss
		[3] = 25,   -- 3 blessings = 25% loss
		[4] = 10,   -- 4 blessings = 10% loss
		[5] = 5     -- 5 blessings = 5% loss (minimal but still some loss)
	}
	
	local basePercent = lossPercent[blessings]
	
	-- VIP blessing reduces to absolute minimum (1%)
	-- With 5 basic blessings: 5% loss
	-- With 5 basic + VIP: 1% loss (always keeps a small loss)
	if hasVipBlessing and blessings == 5 then
		basePercent = 1  -- minimum loss even with max protection
	elseif hasVipBlessing then
		-- For less than 5 blessings, VIP reduces by 40% of base
		basePercent = math.max(1, math.floor(basePercent * 0.6))
	end
...
function Player.updateKillTracker(self, monster, corpse)
	...
	-- First, calculate total item count including nested containers
	local totalItems = 0
	for i = 0, corpse:getSize() - 1 do
		local item = corpse:getItem(i)
		totalItems = totalItems + 1
		if item:isContainer() then
			totalItems = totalItems + item:getSize()
		end
	end

	networkMessage:addByte(totalItems)

	-- Now send all items including container contents
	for i = corpse:getSize() - 1, 0, -1 do
		...
		if item:isContainer() then
			for j = 0, item:getSize() - 1 do
				local nestedItem = item:getItem(j)
				networkMessage:addItem(nestedItem)
				networkMessage:addString(nestedItem:getName())
			end
		end
	end
