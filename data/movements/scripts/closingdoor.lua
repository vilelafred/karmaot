function onStepOut(creature, item, position, fromPosition)
	...
	while tileItem and i < tileCount do
		...
		if tileItem and tileItem:getUniqueId() ~= item.uid then
			local itemType = tileItem:getType()
			-- Remove movable items or magic fields/runes
			if itemType:isMovable() or itemType:getGroup() == ITEM_GROUP_RUNE or tileItem:getId() >= 1487 and tileItem:getId() <= 1506 then
				tileItem:remove()
			else
				i = i + 1
			end
