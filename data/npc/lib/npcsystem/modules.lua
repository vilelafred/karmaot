...
if(Modules == nil) then
	...
	FOCUS_GREETWORDS = {'hi', 'hello', "DJANNI'HAH", "djanni'hah", "PIEDPIPER", "piedpiper"}
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		if premium and parameters.premium and not player:isPremium() then
			npcHandler:say("I'm sorry, but you need a premium account in order to travel onboard our ships.", cid)
		elseif parameters.level and player:getLevel() < parameters.level then
			npcHandler:say("You must reach level " .. parameters.level .. " before I can let you go there.", cid)
		elseif (player:getMoney() + player:getBankBalance()) < cost then
	...
	function TravelModule.bringMeTo(cid, message, keywords, parameters, node)
		...
		-- 15 seconds cooldown for the global bring-me shortcut
		...
		if getPlayerStorageValue(cid, bringExhaust) > now then
			...
			module.npcHandler:say("You must wait " .. remaining .. " second(s) before using the 'bring me to' shortcut again.", cid)
			return true
		end
...
-- Parse a string contaning a set of buyable items.
	...
	function ShopModule:addBuyableItem(names, itemid, cost, charges, realname)
		for i, name in pairs(names) do
			...
			if ItemType(itemid):isRune() or isItemFluidContainer(itemid) == true then
...
function ShopModule.onConfirm(cid, message, keywords, parameters, node)
    local module = parameters.module
    if cid ~= module.npcHandler.focus then return false end
    ...
    elseif parentParameters.eventType == SHOPMODULE_BUY_ITEM then
		...
		-- 💬 Custom message to show how much was used and current bank balance
		...
		-- 🏦 Add current bank balance at the end
