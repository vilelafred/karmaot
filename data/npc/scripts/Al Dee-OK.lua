...
function creatureSayCallback(cid, type, msg)
	...
	npcHandler:say('Crowbar?  are super hard to come by. I trade them only for my . Do you want to trade?')
	talk_state = 2
	
	elseif msgcontains(msg, 'yes') and talk_state == 2 and getPlayerItemCount(cid,4846) >= 1 then
	npcHandler:say('WoW! Here take your crowbar, remember to keep this a secret!')
	doPlayerRemoveItem(cid,4846,1)
	GOBLINROOM = doPlayerAddItem(cid,6051,1)
	doSetItemActionId(GOBLINROOM, 5123)
	talk_state = 0

	elseif msgcontains(msg, 'yes') and talk_state == 2 and getPlayerItemCount(cid,4846) == 0 then
	npcHandler:say('Sorry, I am looking for my Iron Hammer.')
	talk_state = 0
