...
keywordHandler:addKeyword({'iron hammer'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I had an iron hammer, but I lost it while running from an orc spearman.."})
keywordHandler:addKeyword({'levers key'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I found this key.."})
keywordHandler:addKeyword({'goblins'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I hate these creatures, on one occasion I had to fight them."})
...
keywordHandler:addKeyword({'all dee register'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "this is my number one diary!"})
keywordHandler:addKeyword({'princess'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I don't know anything about this princess!"})
...
keywordHandler:addKeyword({'cipfried'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I dont trust him!"})
...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'levers room') then
	npcHandler:say('I would only leave you with the key to this location if you find my iron hammer, accepted?')
	talk_state = 2
	
	elseif msgcontains(msg, 'yes') and talk_state == 2 and getPlayerItemCount(cid,4846) >= 1 then
	npcHandler:say('Im very grateful to have found my iron hammer, take this key! and dont tell anyone about it! its a secret')
	doPlayerRemoveItem(cid,4846,1)
	GOBLINROOM = doPlayerAddItem(cid,2087,1)
	doSetItemActionId(GOBLINROOM, 5123)
	talk_state = 0

	elseif msgcontains(msg, 'yes') and talk_state == 2 and getPlayerItemCount(cid,4846) == 0 then
	npcHandler:say('NO, I am ONLY want for my Iron Hammer.')
	talk_state = 0
