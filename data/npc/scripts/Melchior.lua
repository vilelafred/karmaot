...
function creatureSayCallback(cid, type, msg)
	...
	if(msgcontains(msg:lower(),"djanni'hah")) and (getPlayerStorageValue(cid, 1030) ~= 1) then
    ...
    elseif(msgcontains(msg:lower(),"greeting") or msgcontains(msg:lower(),"word")) and (getPlayerStorageValue(cid, 1030) ~= 1) then
        setPlayerStorageValue(cid, 1030, 1)
        setPlayerStorageValue(cid, 1020, 1)
        npcHandler:say("The djinns have an ancient code of honour. This code includes a special concept of hospitality. Anybody who utters the word of greeting must not be attacked even if he is an enemy. Well, at least that is what the code says. ...")
        npcHandler:say("I have found out, though, that this does not work at all times. There is no point to say the word of greeting to an enraged djinn. ...")
        npcHandler:say("I can tell you the word of greeting if you're interested. It is DJANNI'HAH. Remember this word well, stranger. It might save your life one day. ...")
        npcHandler:say("And keep in mind that you must choose sides in this conflict. You can only follow the Efreet or the Marid - once you have made your choice there is no way back. I know from experience that djinn do not tolerate double-crossing.")
