...
keywordHandler:addKeyword({'red rose'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "this is carlin's secret force, you shouldn't know about it!."})
keywordHandler:addKeyword({'princess'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "are you asking about my daughter?."})
...
function creatureSayCallback(cid, type, msg)
	...
	npcHandler:say('if you possess this sword, you will be worthy of my secrets.')
	talk_state = 2
	
	elseif msgcontains(msg, 'yes') and talk_state == 2 and getPlayerItemCount(cid,4846) >= 1 then
	npcHandler:say('I understand, its worth trusting you access my room and discover my secrets.')
	doPlayerRemoveItem(cid,2395,1)
	CARLINSWORD = doPlayerAddItem(cid,2087,1)
	doSetItemActionId(CARLINSWORD, 5100)
	talk_state = 0
...
local node1 = keywordHandler:addKeyword({'promot'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'Do you want to be promoted in your vocation for 100000 gold?'})
	node1:addChildKeyword({'yes'}, StdModule.promotePlayer, {npcHandler = npcHandler, promotions = {[1] = 5, [2] = 6, [3] = 7, [4] = 8}, cost = 100000, level = 20, text = 'Congratulations! You are now promoted.'})
