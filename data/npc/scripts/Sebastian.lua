...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'nargor') or msgcontains(msg, 'nargor') then
	npcHandler:say('Hey mate! So you are looking for a passage to the nargor for 80 gold?')
	talk_state = 1
	...
	if 1 == 1 then
		if getCreatureCondition(cid, CONDITION_INFIGHT) ~= 1 then
			if getPlayerMoney(cid) >= 80 then
				...
				doTeleportThing(cid, {x = 31916, y = 32873, z = 7})
...
npcHandler:setMessage(MESSAGE_GREET, "Greetings, daring adventurer. If you need a passage to {nargor}, let me know.")
