...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, 'liberty bay') or msgcontains(msg, 'liberty') then
	npcHandler:say('Hey mate! So you are looking for a passage to the liberty bay for 80 gold?')
	talk_state = 1
	...
	if 1 == 1 then
		if getCreatureCondition(cid, CONDITION_INFIGHT) ~= 1 then
			if getPlayerMoney(cid) >= 80 then
				...
				doPlayerRemoveMoney(cid, 80)
				doTeleportThing(cid, {x = 32241, y = 32916, z = 7})
...
npcHandler:setMessage(MESSAGE_GREET, "Greetings, daring adventurer. If you need a passage to {liberty bay}, let me know.")
