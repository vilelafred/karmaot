...
function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, 'yes') and talk_state[cid] == 1 then
		if player:removeMoney(5) then
			local item = player:addItem(2088, 1)
			if item then
				item:setActionId(4600)
				item:setAttribute(ITEM_ATTRIBUTE_DESCRIPTION, "(key: 4600)")
			end
	...
	elseif msgcontains(msg, 'yes') and talk_state[cid] == 2 then
		if player:getItemCount(1990) >= 1 then
			player:removeItem(1990, 1)
			player:addItem(2480, 1)
			npcHandler:say('THANK YOU! Here is a helmet that will serve you well.', cid)
		else
			npcHandler:say('HEY! You don\'t have one! Stop playing tricks or I will give you some extra work!', cid)
		end
		talk_state[cid] = 0
	...
	elseif msgcontains(msg, 'yes') and talk_state[cid] == 3 then
		local count = player:getItemCount(2813)
		if count > 0 then
			player:removeItem(2813, count)
			player:addMoney(count * 2)
			npcHandler:say("Thank you! Here is your reward.", cid)
		else
			npcHandler:say("HEY! You don't have any! Stop playing tricks or I will give you some extra work!", cid)
		end
		talk_state[cid] = 0

	elseif msgcontains(msg, 'fuck') then
		local genderMsg = (player:getSex() == 0) and "young lady" or "young man"
		npcHandler:say("For this remark I will wash your mouth with soap, " .. genderMsg .. "!", cid)
