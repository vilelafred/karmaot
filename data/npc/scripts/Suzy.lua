...
-- Função para formatar gold de forma legível
local function formatGold(amount)
	if amount >= 1000000 then
		-- Converte para milhões (kk) com 2 casas decimais
		local kk = amount / 1000000
		-- Se for número exato de kk, não mostra casas decimais
		if kk == math.floor(kk) then
			return string.format("%dkk", kk)
		else
			return string.format("%.2fkk", kk)
		end
	elseif amount >= 1000 then
		-- Converte para milhares (k) com 2 casas decimais
		local k = amount / 1000
		...
		return string.format("%d gold", amount)
	end
end
...
----BALANCE-------------------------------------------------------------------------
	elseif	msgcontains(msg, 'balance') then
		...
		npcHandler:say("Your account balance is ".. formatGold(balance) ..".", 1)
		talk_state = 0
...
----DEPOSIT------------------------------------------------------------------------
	...
	elseif talk_state == 203 and msgcontains(msg, 'yes') then
		if cid.depositMoney(cid, DEPOSITAMOUNT) == true then
	...
	elseif talk_state == 205 and msgcontains(msg, 'yes') then
		...
		if cid.depositMoney(cid, PLAYERCURRENTMONEY) == true then
...
----WITHDRAW-------------------------------------------------------------------------
	...
	elseif msgcontains(msg, 'withdraw') then
		...
		if WITHDRAWMONEY >= 1 then
			...
			npcHandler:say("How much would you like to withdraw? Say 'all' to withdraw everything or say a specific amount.", 1)
			talk_state = 301
		end
	...
	elseif talk_state == 303 and msgcontains(msg, 'yes') then
		if cid.withdrawMoney(cid, WITHDRAWMONEY) == true then
