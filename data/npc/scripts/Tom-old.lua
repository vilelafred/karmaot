...
keywordHandler:addKeyword({'brother'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "It's been a while since I've seen my brother, I'm worried about him."})
keywordHandler:addKeyword({'brooch'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "Me and my brother got a brooch for our bravery.."})
keywordHandler:addKeyword({'excalibug'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "This legend will never die, will it?.."})
keywordHandler:addKeyword({'sword of fury'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "this sword is the most powerful in rookgaard. but only the worthy can have it..."})
keywordHandler:addKeyword({'amber'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "she's beautiful.."})
keywordHandler:addKeyword({'minotaur mage'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "we are not powerful enough to fight it.."})
...
TOMBROCH = getPlayerItemCount(cid,2826)
if TOMBROCH >= 1 then
	if doPlayerRemoveItem(cid, 2826, TOMBROCH) == true then
	doPlayerAddMoney(cid, TOMBROCH*5)
	npcHandler:say("Deal. By the way: If you'd like to hunt something bigger, check the cellar of the stables to the north. Some adventurer used to store his loot under a loose board beneath a barrel. He might have forgotten something when he left the isle.", 1)
	end
else
npcHandler:say("Sorry, you do not have any fresh one.", 1)
...
elseif msgcontains(msg, 'tomy brooch') or msgcontains(msg, 'tommy brooch') then
	npcHandler:say("Are you saying you found my brother's brooch?", 1)
	talk_state = 5
	
elseif talk_state == 5 and msgcontains(msg, 'yes') then
TOMBROCH = getPlayerItemCount(cid,6138)
if TOMBROCH >= 1 then
	if doPlayerRemoveItem(cid, 6138, TOMBROCH) == true then
	TOMROOM = doPlayerAddItem(cid, 2088, 1)
	doSetItemActionId(TOMROOM, 5124)
	talk_state = 0
	npcHandler:say("thank you so much for bringing me this, it brings me good memories, please take as a reward my brother's things, which are in my room! just remember not to tell anyone, let's keep it a secret, ok?.", 1)
	end
else
npcHandler:say("Sorry, you do not have it.", 1)
end
talk_state = 0
	end
