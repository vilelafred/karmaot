local config = {
    actionId = 64444,
    bossName = "Revenge Incarnate",
    bossPosition = Position(31613, 32026, 0),
    -- Storage usada para liberar o atalho permanente (defina esta mesma AID na porta do atalho)
    shortcutStorage = 64400,
    -- AID aplicada no teleport de saída; ao pisar nele marcamos a storage do atalho
    exitTeleportAid = 64401,
    bossArea = {
        fromPos = Position(31605, 32018, 0), 
        toPos = Position(31619, 32032, 0),
        entrancePos = Position(31613, 32021, 0),
        exitPosition = Position(31612, 32025, 1)
    }, 
    allowedAnyParticipantsCount = true,
    participantsPos = {
        Position(31617, 32023, 1),
        Position(31617, 32024, 1),
        Position(31617, 32025, 1),
        Position(31617, 32026, 1),       
    },
    attempts = {
        level = 700,
        storage = 6644,
    ...
    createTeleportPos = Position(31618, 32023, 0), 
    teleportToPosition = Position(31613, 32021, 8), 
...
function action.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    for _, pos in pairs(config.participantsPos) do
        ...
        if creature and creature:isPlayer() then
            ...
            if creature:getItemCount(6846) < 7 then
                player:sendCancelMessage("All players need 7 Magic Sulphurs to pull the lever.")
                return true
            end
