local config = {
    ...
    playItem = {
        itemId = 6692, -- item required to pull lever
        count = 2
    },
    rouletteOptions = {
    ...
    prizePool = {
        {itemId = 6832, count = {1, 1},    chance = 4 }, -- multi tool
        {itemId = 6155, count = {1, 1},    chance = 8 }, -- training staff
        {itemId = 6698, count = {1, 1},    chance = 3 }, -- exp boost		
        {itemId = 6838, count = {1, 1},    chance = 3 }, -- bless scroll
        {itemId = 6846, count = {1, 10},    chance = 5  },  -- magic sulphur
        {itemId = 6610, count = {1, 1},    chance = 1 }, -- ultimate surprise bag			
        {itemId = 5903, count = {1, 1},    chance = 1 }, -- winged boots +40 speed
        {itemId = 2640, count = {1, 1},    chance = 1 }, -- karma boots	
        {itemId = 8243, count = {1, 1},    chance = 1 }, -- magic feather aura		
        {itemId = 6818, count = {1, 1},    chance = 15 }, -- temple scroll
        {itemId = 6692, count = {1, 1},    chance = 10 }, -- casino ticket 	
        {itemId = 5804, count = {1, 1},   chance = 3}, -- karma ring {itemId = itemid, count = {min, max}, chance = chance/10000} 
        {itemId = 2160, count = {1, 15}, chance = 30 }, -- crystal coin   -- items with 'charges' and have 'showCharges' in items.xml will be given charges - PLATINUM COINS					
    },
	
    roulettePositions = { -- hard-coded to 7 positions.
        Position(32056, 32318, 7),
        Position(32057, 32318, 7),
        Position(32058, 32318, 7),
        Position(32059, 32318, 7), -- position 4 in this list is hard-coded to be the reward location, which is the item given to the player
        Position(32060, 32318, 7),
        Position(32061, 32318, 7),
        Position(32062, 32318, 7),
    }
}
...
function casinoRoulette.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if player:getItemCount(config.playItem.itemId) < config.playItem.count then
     ...
     player:sendTextMessage(TALKTYPE_MONSTER_SAY, "Good luck!")
