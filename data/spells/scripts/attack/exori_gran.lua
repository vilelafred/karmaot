...
function getSpellDamage(cid, weaponSkill, weaponAttack, attackStrength) 
    local hit = (getPlayerLevel(cid) * 1.3 + weaponSkill * 6.0 + weaponAttack * 9.0 + (getPlayerMagLevel(cid)+1) / 3) * 1.61  
    local damage = -(math.random(hit * 0.9, hit)) 
        ...
        if weaponSlot.itemid == 2396 then
            ...
            doPlayerSendTextMessage(cid, MESSAGE_STATUS_WARNING, "Your Ice Rapier broke with the power of exori gran!")
        else
