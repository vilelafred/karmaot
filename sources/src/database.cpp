...
//RETORNA SEM ERRO/LOG NO CONSOLE

std::string DBResult::getString(const std::string& s) const
{
    auto it = listNames.find(s);
    if (it == listNames.end() || row == nullptr || row[it->second] == nullptr) {
        // Retorne um valor padrão ou vazio se a coluna não existir ou se o valor for nulo
        return std::string(); // ou outro valor padrão que faça sentido para o seu código
    }

    return std::string(row[it->second]);
}

// RETORNA COM LOG/ERRO NO CONSOLE

//std::string DBResult::getString(const std::string& s) const
//{
//    auto it = listNames.find(s);
//    if (it == listNames.end()) {
//        std::cout << "[Error - DBResult::getString] Column '" << s << "' does not exist in the result set." << std::endl;
//        return std::string();
//    }

//    if (row == nullptr || row[it->second] == nullptr) {
//        std::cout << "[Error - DBResult::getString] Value in column '" << s << "' is null." << std::endl;
//        return std::string(); // ou outro valor padrão que faça sentido para o seu código
//    }
