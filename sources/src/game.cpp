 ...
 */
 ...
 void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint32_t pos, uint16_t amount, uint64_t price, bool anonymous)
 {
	 bool delayed = false;
	 ...
	 if (type == MARKETACTION_SELL) {
		 ...
		 if (it.stackable) {
			 ...
			 delayed = true;
			 amount = 1; // for non stackable items
		 }
	 ...
	 if(delayed) {
		 ...
		 if (item) {
			 if(item->hasAnyAttributes()) {
				 ...
				 IOMarket::createOffer(player->getGUID(), static_cast<MarketAction_t>(type), it.id, amount, price, anonymous, item->getDescription(0), 0, nullptr); //attributes SAVED with offer
			 }
			 internalRemoveItem(item);
		 }
	 }
	 ...
	 if (MarketAction_t::MARKETACTION_BUY == type) {
		IOMarket::createOffer(player->getGUID(), static_cast<MarketAction_t>(type), it.id, amount, price, anonymous, "", 0, nullptr); //attributes SAVED with offer
	 }
	 ...
	 if (offer.type == MARKETACTION_BUY) {
		 ...
		 if (it.stackable) {
			 ...
			 for (uint16_t i = 0; i < offer.amount; ++i) {
				 ...
				 propStream.init(offer.attr, offer.attrSize);
				 if (!item->unserializeAttr(propStream)) {
					 std::cout << "WARNING: Serialize error in Game::playerCancelMarketOffer" << std::endl;
				 }
				 if (internalAddItem(player->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
					 delete item;
					 break;
	 ...
	 if (offer.type == MARKETACTION_BUY) {
		 ...
		 if (it.stackable) {
			 ...
			 for (uint16_t i = 0; i < amount; ++i) {
				 ...
				 propStream.init(offer.attr, offer.attrSize);
				 if (!item->unserializeAttr(propStream)) {
					 std::cout << "WARNING: Serialize error in Game::playerAcceptMarketOffer" << std::endl;
				 }
				 if (internalAddItem(buyerPlayer->getInbox(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RETURNVALUE_NOERROR) {
					 delete item;
					 break;
 ...
 std::forward_list<Item*> Game::getMarketItemList(uint16_t clientId, uint16_t sufficientCount, DepotChest* depotChest, Inbox* inbox)
 {
