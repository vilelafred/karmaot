 ...
 */
 ...
 MarketOfferList IOMarket::getActiveOffers(MarketAction_t action, uint16_t itemId)
 {
	 ...
	 DBResult_ptr result = Database::getInstance().storeQuery(fmt::format("SELECT `id`, `amount`, `price`, `created`, `anonymous`, `description`, `attributes`,(SELECT `name` FROM `players` WHERE `id` = `player_id`) AS `player_name` FROM `market_offers` WHERE `sale` = {:d} AND `itemtype` = {:d}", action, itemId));
	 if (!result) {
		 return offerList;
	 }
 ...
 MarketOfferList IOMarket::getOwnOffers(MarketAction_t action, uint32_t playerId)
 {
	 ...
	 DBResult_ptr result = Database::getInstance().storeQuery(fmt::format("SELECT `id`, `amount`, `price`, `created`, `itemtype`, `description` FROM `market_offers` WHERE `player_id` = {:d} AND `sale` = {:d}", playerId, action));
	 if (!result) {
		 return offerList;
	 }
