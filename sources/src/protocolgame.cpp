		...
		if (!player->hasFlag(PlayerFlag_CannotBeBanned)) {
			...
			if (IOBan::isAccountBanned(accountId, banInfo)) {
				...
				if (banInfo.expiresAt > 0) {
					disconnectClient(fmt::format("Your account has been banned until {:s}{:s}.\n\nReason specified:\n{:s}", formatDateShort(banInfo.expiresAt), banInfo.bannedBy, banInfo.reason));
				} else {
					disconnectClient(fmt::format("Your account has been permanently banned{:s}.\n\nReason specified:\n{:s}", banInfo.bannedBy, banInfo.reason));
				}
				return;
			}
		}
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if (IOBan::isIpBanned(getIP(), banInfo)) {
		...
		disconnectClient(fmt::format("Your IP has been banned until {:s}{:s}.\n\nReason specified:\n{:s}", formatDateShort(banInfo.expiresAt), banInfo.bannedBy, banInfo.reason));
		return;
	}
...
void ProtocolGame::sendMarketEnter(uint32_t depotId)
{
	...
	std::map<uint16_t, uint32_t> depotPositions;
	...
	for (Item* item : depotChest->getItemList()) {
			...
			depotItems[itemType.clientId] += Item::countByType(item, -1);
			depotPositions[itemType.clientId] = item->getRealUID();
	...
	for (std::map<uint16_t, uint32_t>::const_iterator it = depotItems.begin(); i < itemsToSend; ++it, ++i) {
		...
		msg.add<uint32_t>(depotPositions[it->first]);
